...
# please make sure to elect the server that will work on ur local IDE, in my case Alex Sylvain i selcted "Cloud Code Yaml support" on output
import pandas as pd  # pip install panda
import datetime
import smtplib  # pip install smtplib
import time
import requests
from win10toast import ToastNotifier  # pip install win10toast  if possibvle you can also install ToastNotifier
...
def sendEmail(to, sub, msg):
    ...
    print("Email sent to " + str(to) + " with subject " + str(sub) +
          " and message :" + str(msg))
...
def sendsms(to, msg, name, sub):
    ...
    print("SMS sent to " + str(to) + " with subject :" + str(sub) +
          " and message :" + str(msg))
